...
classdef RML < handle
    ...
    methods
        ...
        function learn(obj,rw,s,s1,resp,RW,b)%outcome analysis and learning           
            
            if obj.omega==0 %if it's ACC Action
                
               R=rw*obj.lesion*(RW+(obj.mu*b)); %primary reward boost modulation
               obj.D(s,resp)=(R+(1-obj.mu)*b*obj.gamma*obj.lesion*max(obj.V(s1,:))-obj.V(s,resp)); %TD learning for Action
               
            else %if it's ACC Boost
                
               R=rw*RW*obj.lesion-obj.omega*b; %primary reward boost cost
               obj.D(s,resp)=(R+obj.lesion*max(obj.V(s1,:))-obj.V(s,resp)); %TD learning for Boost
